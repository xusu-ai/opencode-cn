...
`,
  ...
  try {
    ...
    await expect(fs.readFile(tmp.extra.marker, "utf8")).rejects.toThrow()
    expect(TuiPluginRuntime.list().some((item) => item.spec === tmp.extra.spec)).toBe(false)
    // Only check for errors from the plugin under test, not internal plugins (e.g., home-tips i18n context)
    const pluginErrors = error.mock.calls.filter((call) => {
      const msg = String(call[0])
      return !msg.includes("internal:home-tips") && !msg.includes("I18n context")
    })
