...
`,
  ...
  try {
    ...
    // no initialization failures (exclude internal plugins that need i18n context)
    const hit = err.mock.calls.find(
      (item) => typeof item[0] === "string" && item[0].includes("failed to initialize tui plugin") && !item[0].includes("internal:home-tips") && !item[0].includes("I18n context"),
    )
