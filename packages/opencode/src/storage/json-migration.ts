...
export async function run(db: SQLiteBunDatabase<any, any> | NodeSQLiteDatabase<any, any>, options?: Options) {
  ...
  try {
    ...
    log.warn("failed to set migration pragmas", { error: String(err) })
  }
  const stats = {
    projects: 0,
    sessions: 0,
  ...
  async function list(pattern: string) {
    try {
      return await Glob.scan(pattern, { cwd: storageDir, absolute: true })
    } catch (err) {
      errs.push(`failed to scan ${pattern}: ${err}`)
      return []
    }
  }
  ...
  db.run("COMMIT")
    ...
    try {
      ...
      log.warn("failed to rollback transaction", { error: String(rollbackErr) })
    }
    errs.push(`transaction failed: ${err}`)
    log.warn("migration transaction failed, rolled back", { error: String(err) })
  }
