...
> {
  ...
  switch (result.type) {
    case MaybeApplyPatch.Body:
      ...
      // Security: Validate workdir doesn't escape project boundary
      const normalizedCwd = path.resolve(cwd)
      const normalizedEffectiveCwd = path.resolve(effectiveCwd)
      if (!normalizedEffectiveCwd.startsWith(normalizedCwd + path.sep) && normalizedEffectiveCwd !== normalizedCwd) {
        return {
          type: MaybeApplyPatchVerified.CorrectnessError,
          error: new Error(`Workdir escapes project directory: ${args.workdir}`),
        }
      }
      ...
      for (const hunk of args.hunks) {
        ...
        // Security: Validate each resolved path stays within the project directory
        const normalizedResolvedPath = path.resolve(resolvedPath)
        if (!normalizedResolvedPath.startsWith(normalizedCwd + path.sep) && normalizedResolvedPath !== normalizedCwd) {
          return {
            type: MaybeApplyPatchVerified.CorrectnessError,
            error: new Error(`Path escapes project directory: ${hunk.path}`),
          }
        }
