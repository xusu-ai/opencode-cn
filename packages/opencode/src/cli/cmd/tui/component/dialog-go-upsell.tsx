      ...
      <box paddingLeft={PAD_X} paddingRight={PAD_X} paddingBottom={1} gap={1}>
        ...
        <box ref={(item: BoxRenderable) => (descBox = item)} gap={0}>
          <box flexDirection="row">
            <text fg={theme.textMuted}>{t("tui.goUpsell.subscribeTo")}</text>
            <text attributes={TextAttributes.BOLD} fg={theme.textMuted}>
              OpenCode Go
            </text>
            <text fg={theme.textMuted}> {t("tui.goUpsell.descriptionLine1")}</text>
          </box>
          <text fg={theme.textMuted}>{t("tui.goUpsell.descriptionLine2")}</text>
        </box>
