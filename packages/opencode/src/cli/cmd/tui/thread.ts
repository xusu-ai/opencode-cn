  ...
  handler: async (args) => {
    ...
    try {
      ...
      if (args.fork && !args.continue && !args.session) {
        UI.error(en["tui.attach.forkRequiresContinueOrSession"])
        process.exitCode = 1
        return
      }
      ...
      try {
        ...
        UI.error(en["tui.thread.failedToChdir"] + next)
        return
      }
      ...
      const error = (e: unknown) => {
        ...
        // Write crash dump for unhandled errors
        try {
          const dir = path.join(homedir(), ".opencode")
          if (!existsSync(dir)) mkdirSync(dir, { recursive: true })
          const timestamp = new Date().toISOString().replace(/[:.]/g, "-")
          const filePath = path.join(dir, `crash-dump-${timestamp}.json`)
          const dump = {
      ...
      // Auto-update disabled: checkUpgrade call removed.
      // setTimeout(() => {
