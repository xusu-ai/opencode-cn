...
function state(api: TuiPluginApi, item: TuiPluginStatus, t: ReturnType<typeof useI18n>["t"]) {
  if (!item.enabled) {
...
function meta(item: TuiPluginStatus, width: number, t: ReturnType<typeof useI18n>["t"]) {
  if (item.source === "internal") {
    if (width >= 120) return t("tui.plugin.builtInPlugin")
    return t("tui.plugin.builtIn")
  }
...
function Install(props: { api: TuiPluginApi }) {
  ...
  return (
    <props.api.ui.DialogPrompt
      title={t("tui.plugin.installPlugin")}
      placeholder={t("tui.plugin.npmPackagePlaceholder")}
      busy={busy()}
      busyText={t("tui.plugin.installingPlugin")}
      description={() => (
        <box flexDirection="row" gap={1}>
          ...
          <Show when={!busy()}>
            <text fg={props.api.theme.current.textMuted}>({Keybind.toString(tab)} {t("tui.plugin.toggleHint")})</text>
          </Show>
        </box>
      )}
      onConfirm={(raw) => {
        ...
        if (!mod) {
          props.api.ui.toast({
            variant: "error",
            message: t("tui.plugin.packageNameRequired"),
          })
          return
        }
        ...
        void props.api.plugins
          ...
          .then((out) => {
            ...
            props.api.ui.toast({
              variant: "success",
              message: t("tui.plugin.installedMod", { mod, scope: global() ? t("tui.plugin.global") : t("tui.plugin.local"), dir: out.dir }),
            })
            if (!out.tui) {
              props.api.ui.toast({
                variant: "info",
                message: t("tui.plugin.noTuiTarget"),
              })
              show(props.api)
              return
            }
            ...
            return props.api.plugins.add(mod).then((ok) => {
              if (!ok) {
                props.api.ui.toast({
                  variant: "warning",
                  message: t("tui.plugin.loadFailed"),
                })
                show(props.api)
                return
              }
              ...
              props.api.ui.toast({
                variant: "success",
                message: t("tui.plugin.loadedInSession", { mod }),
              })
              show(props.api)
            })
          })
...
function row(api: TuiPluginApi, item: TuiPluginStatus, width: number, t: ReturnType<typeof useI18n>["t"]): DialogSelectOption<string> {
  return {
    title: item.id,
    value: item.id,
    category: item.source === "internal" ? t("tui.plugin.category.internal") : t("tui.plugin.category.external"),
    ...
    void task
      .then((ok) => {
        if (!ok) {
          props.api.ui.toast({
            variant: "error",
            message: t("tui.plugin.failedToUpdate", { id: item.id }),
          })
        }
...
const tui: TuiPlugin = async (api) => {
  api.command.register(() => {
    // Must call useI18n inside a reactive context; command register callback
    // runs within the SolidJS component tree, so we can safely call it here
    const { t } = useI18n()
    return [
      {
        title: t("tui.plugin.title"),
