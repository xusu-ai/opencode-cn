...
export function formatPart(part: Part, options: TranscriptOptions): string {
  ...
  if (part.type === "tool") {
    ...
    if (options.toolDetails && part.state.status === "completed" && part.state.output) {
      // Security: Strip ANSI control sequences from tool output before including in transcript
      // This prevents ANSI injection when transcript is copied to clipboard or saved to file
      result += `\n**Output:**\n\`\`\`\n${stripAnsi(part.state.output)}\n\`\`\`\n`
    }
    if (options.toolDetails && part.state.status === "error" && part.state.error) {
      // Security: Strip ANSI from error output as well
      result += `\n**Error:**\n\`\`\`\n${stripAnsi(part.state.error)}\n\`\`\`\n`
    }
    result += `\n`
    return result
  }
