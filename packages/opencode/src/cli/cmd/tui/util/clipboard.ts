...
export async function read(): Promise<Content | undefined> {
  ...
  if (os === "darwin") {
    // Security: Use random suffix for temp file to prevent symlink/T0CT0U race attacks
    // Previously used predictable "opencode-clipboard.png" which could be symlinked
    const tmpfile = path.join(tmpdir(), `opencode-clipboard-${crypto.randomBytes(8).toString("hex")}.png`)
    try {
      await Process.run(
        [
          "osascript",
