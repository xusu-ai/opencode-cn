  ...
  async handler(args) {
    await Instance.provide({
      ...
      async fn() {
        ...
        if (entry?.tokens) {
          // Security: Only show first 8 chars of token to minimize exposure in logs
          prompts.log.info(`  Access token: ${entry.tokens.accessToken.substring(0, 8)}...${entry.tokens.accessToken.slice(-4)}`)
